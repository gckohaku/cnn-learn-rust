...
pub fn mnist_process() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for epoch in 1..=epoch_value {
        ...
        let epoch_learning_rate = max_learning_rate * ElementType::powf(0.02, ((epoch - 1) % 10) as ElementType / 9.0);
        ...
        for indices in shuffle_index.chunks_exact(mini_batch_sample_size as usize) {
            ...
            print!("\rmini batch count: {} / {}", mini_batch_count, mini_batches_per_epoch);
